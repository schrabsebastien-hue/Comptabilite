...
export default function ConfirmModal({
    ...
    const getVariantStyles = () => {
        switch (variant) {
            case 'warning':
                return {
                    iconBg: 'bg-amber-500/15 border-amber-500/30 text-amber-700 dark:text-amber-400',
                    iconGlow: 'bg-amber-500/10',
                    confirmBtn: 'bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-white dark:text-slate-950 font-bold shadow-lg shadow-amber-500/20',
                    defaultIcon: MessageSquareX,
                };
            case 'primary':
                return {
                    iconBg: 'bg-indigo-500/15 border-indigo-500/30 text-accent',
            ...
            default:
                return {
                    iconBg: 'bg-rose-500/15 border-rose-500/30 text-negative',
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-surface/80 backdrop-blur-md animate-in fade-in duration-200">
            ...
            >
                ...
                <button
                    onClick={onClose}
                    className="absolute top-4 right-4 text-on-surface-faint hover:text-on-surface p-1.5 rounded-xl hover:bg-surface-overlay transition cursor-pointer"
                >
                    <X className="w-4 h-4" />
                </button>
                ...
                <div className="p-6 space-y-5">
                    ...
                    <div className="flex items-center justify-end space-x-3 pt-2 border-t border-edge/40">
                        <button
                            type="button"
                            onClick={onClose}
                            className="px-4 py-2 rounded-xl text-xs font-semibold text-on-surface-secondary hover:text-on-surface bg-surface-elevated/80 hover:bg-surface-overlay border border-edge-strong transition cursor-pointer"
                        >
                            {cancelText}
                        </button>
