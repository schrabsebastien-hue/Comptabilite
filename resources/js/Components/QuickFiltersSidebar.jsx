...
export default function QuickFiltersSidebar({
    ...
    if (isCollapsed) {
        return (
            <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-2.5 flex flex-col items-center space-y-4 shadow-xl backdrop-blur-sm self-start transition-all">
                <button
                ...
                <button
                    ...
                    className={`p-2.5 rounded-xl transition cursor-pointer relative ${
                        selectedMonth ? 'bg-indigo-600/20 text-indigo-400 border border-indigo-500/30' : 'text-slate-400 hover:bg-slate-800'
                    }`}
                ...
                <button
                    ...
                    className={`p-2.5 rounded-xl transition cursor-pointer ${
                        amountType !== 'all' ? 'bg-indigo-600/20 text-indigo-400 border border-indigo-500/30' : 'text-slate-400 hover:bg-slate-800'
                    }`}
                ...
                {hasActiveFilters && (
                    <button
                        ...
                        className="p-2.5 rounded-xl bg-rose-500/15 text-rose-400 hover:bg-rose-500/25 transition cursor-pointer mt-auto"
    ...
    return (
        <aside className="w-full lg:w-72 shrink-0 space-y-5 bg-slate-900/80 border border-slate-800 rounded-2xl p-4 shadow-xl backdrop-blur-sm transition-all self-start">
            ...
            <div className="space-y-2.5">
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-1.5 text-xs font-semibold text-indigo-400 uppercase tracking-wider">
                    ...
                    <button
                        ...
                        className={`w-full text-left px-3 py-2 rounded-xl text-xs font-medium transition flex items-center justify-between cursor-pointer border ${
                            selectedMonth === null
                                ...
                                : 'bg-slate-950/40 text-slate-300 border-slate-800/80 hover:bg-slate-800/60 hover:text-white'
                        }`}
                    >
                        <div className="flex items-center space-x-2">
                            <span className={`w-2 h-2 rounded-full ${selectedMonth === null ? 'bg-indigo-400' : 'bg-slate-600'}`} />
                            <span>Tous les mois</span>
                        </div>
                        <span className="px-2 py-0.5 rounded-md text-[10px] bg-slate-800 text-slate-400 font-mono">
                            {operations.length}
                        </span>
                    </button>
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full text-left px-3 py-2 rounded-xl text-xs font-medium transition flex items-center justify-between cursor-pointer border group ${
                                        isSelected
                                            ? 'bg-gradient-to-r from-indigo-600/25 to-purple-600/20 text-white border-indigo-500/50 shadow-md shadow-indigo-500/10'
                                            : 'bg-slate-950/40 text-slate-300 border-slate-800/80 hover:bg-slate-800/60 hover:border-slate-700 hover:text-white'
                                    }`}
                                >
                                    <div className="flex items-center space-x-2 min-w-0">
                                        <div
                                            className={`w-2 h-2 rounded-full shrink-0 transition ${
                                                isSelected ? 'bg-indigo-400 scale-110' : 'bg-slate-600 group-hover:bg-indigo-400'
                                            }`}
                                        />
                                    ...
                                    <div className="flex items-center space-x-1.5 shrink-0">
                                        <span
                                            className={`text-[10px] font-mono font-medium ${
                                                item.balance >= 0 ? 'text-emerald-400/90' : 'text-slate-400'
                                            }`}
                                        >
                    ...
                    <button
                        ...
                        className={`py-1.5 px-2 rounded-xl text-xs font-medium text-center transition flex items-center justify-center space-x-1 cursor-pointer border ${
                            amountType === 'credit'
                                ...
                                : 'bg-slate-950/40 text-slate-400 border-slate-800 hover:bg-slate-800 hover:text-emerald-400'
                        }`}
                    >
                    ...
                    <button
                        ...
                        className={`py-1.5 px-2 rounded-xl text-xs font-medium text-center transition flex items-center justify-center space-x-1 cursor-pointer border ${
                            amountType === 'debit'
                                ...
                                : 'bg-slate-950/40 text-slate-400 border-slate-800 hover:bg-slate-800 hover:text-rose-400'
                        }`}
                    >
